...
author: rs
date: '2020-07-21'
slug: austrian-parliament-mps-transparency-and-conflict-of-interest-report
categories: []
tags:
  - Austria
  - OCR
...
And lists of MPs' **2) executive positions** in stock companies, private limited companies, foundations or saving banks held by each MP; **3) other activities** from which MPs generate income; and **4) honorary activities** which yield no income (but could potentially nevertheless lead to some conflict of interest).

Extracting data from pdfs has been the topic of at least one of my previous posts (see [here](https://werk.statt.codes/post/bih-voting-behavior-of-mps-in-house-of-representatives/){target="_blank"}), and again the primary tool for the job is the wonderful `pdftools` package. Rather than seeking to extract tables from the pdf (e.g. with `tabulizer` ), I convert the content of each page of the report to a long text string and subsequently extract the parts of interest by means of `stringr` and regex. However, as I'll explain below, this approach was only suited to do some part of the task.
...
The code above provides us with a tibble containing as many rows as the report has pages (186). The expression `str_replace_all(text_raw, "\\h+", " ")` replaces all horizontal white space with a single spaces, but keeps line breaks (\\n and \\r). The latter will be needed at a later stage when it comes to identifying the location of data of interest and extract it.^[I am grateful to the respondents to my pertaining question on [stackoverflow](https://stackoverflow.com/questions/62678640/stringr-remove-multiple-spaces-but-keep-linebreaks-n-r/62679016#62679016){target="_blank"}]
...
df_mp_dates <- readr::read_rds(path=here::here("blog_data", "MP_activities", "MP_dates.rds"))
...
Let's have a gender-sensitive look at the income data. With the `gender`package we are able to infer with some acceptable margin of error MPs' gender from their first names. 
...
```{r echo=FALSE}
datatable(df_income %>% select(name, party, income_category_19),
...
pl_rel <- df_agg %>% 
  ggplot()+
  labs(title="Relative number of MPs per income category and party",
    subtitle="% are share of each party's total (total = 100%); Members of Austrian lower chamber (Nationalrat); for the calendar year 2019.",
    x="[income category] average monthly income",
    caption=my_caption)+
...
This function is applied to our previous results of the `pdf_data` function. At the end, I combine these results with MPs' names and aggregate those reports of MPs which run over multiple pages. E voila! You can use the arrows to sort the table and get e.g the top-10 MPs as to their number of executive positions.
...
Let's have a look at MPs' number of paid positions and income category. Hover over the points in the plot below to get details on each individual MP. Note that the points are located randomly *within* each square to avoid overplotting.
...
pl_bar_position <- df_bar_position %>% 
  ggplot()+
  labs(title="Percentage of MPs with at least one additional professional position per party",
       subtitle="% are share of each party's total number of MPs. Absolute number in brackets. Only lower chamber. Data for 2019.",
       caption=my_caption)+
  geom_bar(aes(x=party,
               y=perc,
               fill=party),
...
pl_box <- df_position_long %>% 
  ...
  labs(caption=my_caption,
       title="Number of MPs' other professional positions per party",
       subtitle="Members of lower chamber only (Nationalrat); Data for 2019.",
