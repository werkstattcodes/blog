...
df_orf_long <- df_orf %>% 
  ...
  #tidyr::replace_na(., 0)
  pivot_longer(cols=-status, names_to="date", values_to="value") %>% 
  mutate(date=lubridate::mdy(date),
         value=as.numeric(value))

class(df_orf_long$date)
...
pl_tests_v_confirmed <- df_data %>% 
  ...
  labs(title="Covid-19/Austria: \nDaily increase in testing vs Daily increase of confirmed cases",
       ...
       x="% increase of number of tests (day to day)",
       y="% increase of confirmed Covid-19 cases (day to day)")+
  geom_abline(slope = 1)+
...
pl_daily_results_rel<- df_data  %>% 
  ...
  select(status, diff.day, date) %>% 
  pivot_wider(id_cols=date, values_from=diff.day, names_from=status) %>% 
...
pl_line <- df_data %>% 
  filter(status %in% c("confirmed", "tested")) %>% 
  select(status, date, diff.day.perc, value) %>% 
  ggplot()+
  ...
  geom_text_repel(data=. %>%  filter(date==max(df_data$date)),
            aes(x=date,
                y=diff.day.perc,
                color=status,
                label=percent(diff.day.perc, accuracy=0.01)),
            stat="identity",
           # linetype="dotted",
          #  segment.color = "black",
            hjust=0,
