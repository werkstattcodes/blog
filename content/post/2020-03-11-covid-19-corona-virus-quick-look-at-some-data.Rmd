...
orf_timestamp_tests <- df_orf_content %>% 
  ...
  select(Bundesland) %>% 
  mutate(orf_timestamp=lubridate::ymd_hms(Bundesland)) %>% 
  pull(orf_timestamp)
...
df_orf_long <- df_orf %>% 
  mutate(gkz=case_when(gkz=="Zeitlinie" ~ "status",
                       TRUE ~ as.character(gkz))) %>% 
...
df_data <- df_orf_long %>% 
  group_by(status) %>% 
  arrange(date, .by_group=T) %>% 
  mutate(value_day=value-lag(value),
         value_day_diff_perc=value_day/lag(value_day)-1,
         value_accum_diff_perc=(value/lag(value)-1))
...
##ggiraf version
pl_tests_v_confirmed<- df_tests_v_confirmed %>% 
  ggplot()+
...
df_new <- df_tests_v_confirmed %>% 
  group_split(date) %>% 
  accumulate(~ bind_rows(.x, .y)) %>% 
  bind_rows(., .id="frame") %>% 
  mutate(frame=as.numeric(frame)) %>% 
  group_by(frame) %>% 
  mutate(label=paste0(format(max(date), '%a, %d %b %Y'),
                      "\n",
                      "change confirmed: ", percent(last(confirmed, date), accuracy = .01,
                                             scale=100),
                     "\n",
                      "change tested: ", percent(last(tested, date), accuracy = .01,
                                          scale=100))) %>% 
  mutate(size=case_when(date==last(date) ~ "current",
                         TRUE ~ as.character("preceding")),
         alpha=case_when(date==last(date) ~ "current",
                         TRUE ~ as.character("preceding"))) %>% 
  ungroup()
...
```{r eval=FALSE, fig.height=6, fig.width=10, message=FALSE, include=FALSE}
pl_tests_v_confirmed_anim <- df_new %>% 
  arrange(frame) %>% 
  ggplot()+
  labs(title="Covid-19/Austria: \nDaily increase in testing vs Daily increase of confirmed cases",
       subtitle="Each dot represents one day's % change in tests and cases.<br><span style=color:red>Above</span> diagonale line: confirmed <span style=color:red>cases increase faster</span> than number of tests<br><span style=color:black>Below</span> diagonale line: confirmed <span style=color:black>cases increase slower</span> than number of tests",
...
#  labs(title='{closest_state}')+
  ...
  geom_text(aes(label=label),
            ...
            family="Roboto Condensed",
            check_overlap = T)+
  scale_color_manual(values=c("above"="red",
                              "not above"="black"))+
  scale_alpha_manual(values=c("preceding"=.5,
                              "current"=1))+
  scale_size_manual(values=c("preceding"=2,
                              "current"=3))+
  scale_x_percent()+
  scale_y_percent()+
  theme_ipsum_rc()+
...
pl_tests_accumm<- df_data  %>% 
  ...
  pivot_wider(id_cols=date, values_from=value, names_from=status) %>% 
  mutate(healthy=tested-confirmed) %>% 
  select(-tested) %>% 
  pivot_longer(cols=-date, names_to="status", values_to="val") %>%
  group_by(date) %>% 
